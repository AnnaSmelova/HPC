...
double CalcMatVecMulTime_ji(double * A, double * B, double * C, size_t N)
{
    struct timeval start, end;
    double r_time = 0.0;
    size_t i, j;
...
// ij ordering
    ...
    {
        runtimes[n]=CalcMatVecMulTime_ij(&A[0], &V[0], &C[0], N);
        printf("runtime %lf seconds\n", runtimes[n]);
        average_runtime += runtimes[n]/NRuns;
    }

    printf("average runtime ij matrix by vector %lf seconds\n", average_runtime);
    printf("---------------------------------\n");
...
// ji ordering
    ...
    for(int n=0; n<NRuns; n++)
    {
        runtimes[n]=CalcMatVecMulTime_ji(&A[0], &V[0], &C[0], N);
        printf("runtime %lf seconds\n", runtimes[n]);
        average_runtime += runtimes[n]/NRuns;
    }

    printf("average runtime ji matrix by vector %lf seconds\n", average_runtime);
    printf("---------------------------------\n");
